...
export default function CountdownDeli({ deliveryDatesList }) {
    ...
    useEffect(() => {
        ...
        const calculateDaysToGo = () => {
            const updatedDaysToGoList = deliveryDatesList.map((targetDate) => {
    ...
    return (
        <div className='my-3'>
            <div className="board-card-alert flex flex-col justify-center items-center gap-2">
                <div>
                    You have <b style={{ color: 'red', fontSize: '18px' }}> {fewDaysLeft.length}</b> pending <b>DELIVERIES</b> in <b style={{ color: 'red', fontSize: '16px' }}> less</b> than <b style={{ color: 'red', fontSize: '16px' }}>2</b> days.
                </div>
            </div>
        </div>
    )
