...
export default async function handle(req, res) {
    ...
    if (method === 'POST') {
        try {
            const { projectID, standar, vintage, volumen, name, projectLink, tech, corsia, pais, disponible, notas, files } = req.body;
            const projectDoc = await Project.create({
                projectID, standar, vintage, volumen, name, projectLink, tech, corsia, pais, notas, files, disponible
            })
            res.json(projectDoc);
        } catch (error) {
            console.log(error);
    ...
    if (method === 'PUT') {
        const { projectID, standar, vintage, volumen, name, projectLink, tech, corsia, pais, disponible, notas,files, _id} = req.body;
        ...
        await Project.updateOne({ _id }, { projectID, standar, vintage, volumen, name, projectLink, tech, corsia, pais,disponible, notas, files });
        res.json(true);
    }
